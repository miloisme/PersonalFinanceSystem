...
**A local-first, privacy-oriented personal finance desktop application** 🖥️
...
Personal Finance System is a desktop finance application built with **Python + PySide6 (Qt)** 🐍⚡. All data is stored in a local **SQLite** database 🗄️ and can optionally be protected with **AES-256-GCM** encryption 🔐. It offers multi-currency management 💱, income/expense tracking 📝, debt and receivable tracking 💳, budgeting 📊, net-worth charts 📈, Google Drive sync ☁️, and a direct SQL console 💻 -- a complete toolkit for users who want full control over their personal finances.
...
| ⚖️ **Balance** | View per-account and overall net worth in a base currency, with automatic cross-currency conversion. Yearly review with monthly category matrix and sparkline trends. |
...
- ☁️ **Google Drive sync**: Upload/download your encrypted database to Google Drive. Only ciphertext ever leaves your device.
...
|-------|-----------|
| 🐍 **Language** | Python 3.8+ |
| 🖼️ **GUI Framework** | PySide6 (Qt 6) with Fusion style |
| 📊 **Charts** | Matplotlib (embedded via `FigureCanvasQTAgg`) |
| 🗄️ **Database** | SQLite (via the standard `sqlite3` module) |
| 🔐 **Encryption** | `cryptography` package (AES-GCM + PBKDF2) |
| 💱 **Exchange Rates** | `open.er-api.com` (free REST API) |
| ☁️ **Cloud Sync** | Google Drive API v3 via `google-api-python-client` |
...
├── database.py           # 🗄️ SQLite data-access layer (accounts, categories, transactions, debts, settings, ...)
...
├── balance_tab.py        # ⚖️ Balance tab (yearly review, monthly matrix, sparklines)
...
├── settings_tab.py       # ⚙️ Settings tab (currencies, categories, encryption, export)
├── table_utils.py        # 📋 Table sorting / filtering / column-width persistence helpers
├── font_utils.py         # 🔤 CJK font configuration helpers
├── drive_sync.py         # ☁️ Google Drive upload/download (encrypted sync only)
├── requirements.txt      # 📦 Python dependencies
├── .gitignore            # 🚫 Git ignore rules
├── finance.db            # 🗄️ Plaintext database (created on first run)
...
| `categories` | `id`, `name`, `type` (expense/income), `icon` (emoji), `color` (hex) | 🏷️ Income/expense categories |
...
| `settings` | `key`, `value` | ⚙️ Key-value configuration store |
...
### 🏷️ Default Categories (seeded on first run)

| 💵 Income | 💸 Expense |
|--------|---------|
| 💰 Salary | 🍔 Food & Dining |
| 🎁 Bonus | 🚗 Transportation |
| 📈 Investment Income | 🛍️ Shopping |
...
| 🗑️ Secure delete | 3-pass random overwrite with retry logic for Windows file locks |

### 🔄 How Encryption Works

1. ⚙️ **Enable**: Go to Settings > Database Encryption, enter and confirm a password, click "Apply Encryption Settings"
...
4. 🗑️ **Secure wipe**: The plaintext file is overwritten with random data and deleted
...
> ⚠️ **Warning:** A lost password makes the encrypted database unrecoverable.
...
| ⬆️ Upload | Exports clean snapshot via `VACUUM INTO`, encrypts, uploads both files, records MD5 and device name |
| ⬇️ Download | Downloads `.enc` and `.meta`, verifies password matches via verifier, decrypts over local DB |
| ⚠️ Conflict detection | Compares remote `updated_at` with local last sync time |
| 🔒 Security | **Only ciphertext is ever sent to Google** |

### 🔧 Setup

1. 🏗️ Create a Google Cloud project and enable the Drive API
...
- ⚙️ Configurable base currency (default: HKD) -- all views auto-convert
...
- ☁️ **Sync to Google Drive**: Keep your encrypted database in the cloud for cross-device access.
...
|---------|---------|---------|
| `PySide6` | >= 6.5.0 | 🖼️ Qt GUI framework |
| `matplotlib` | >= 3.7.0 | 📊 Charts and visualization |
| `cryptography` | >= 41.0.0 | 🔐 AES-256 encryption (PBKDF2 + AES-GCM) |
| `google-api-python-client` | >= 2.100.0 | ☁️ Google Drive API (optional, for cloud sync) |
